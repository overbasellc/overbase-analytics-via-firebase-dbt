...
SELECT    TIMESTAMP_MICROS(event_timestamp) as created_at
        ...
        , STRUCT<original_language_value STRING, language_iso_alpha_2 STRING, language_region_iso_alpha_2 STRING, time_zone_offset STRING>(
            device.language, language_codes.alpha_2, language_region_codes.alpha_2, IF(device.time_zone_offset_seconds >= 0,'+', '-') || LEFT(CAST(TIME(TIMESTAMP_SECONDS(ABS(device.time_zone_offset_seconds))) AS STRING),5)
        ) AS device
        , STRUCT<city STRING , original_country_value STRING, country STRING , country_iso_alpha_2 STRING, continent STRING, region STRING, sub_continent STRING, metro STRING>(
